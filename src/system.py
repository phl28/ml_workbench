...
with col2:
    st.markdown(
        '<p class="small-font">Platform for general use, both with medical related and general engineering data sources to aid the analysis of data through the use of Neural Networks.',
        unsafe_allow_html=True,
    )
...
if (
    file_dyn is not None
    and file_vit is not None
    and file_vid is not None
    and real_path is not None
):
    # Process input data
        ...
        for key, value in dyn_dict.items():
            if "acc" in key:
                ...
                df_temp1 = pd.DataFrame(
                    {
                        "idx": np.arange(0, df_temp["idx"].max() + 1 / 20, 1 / 20),
                    }
                )
                df_temp1[str_temp] = np.interp(
                    df_temp1.idx, df_temp.idx, df_temp[str_temp]
                )
                df_temp1["key"] = key
    ...
    d = {0.0: "Down", 1.0: "Run", 2.0: "Sit", 3.0: "Up", 4.0: "Walk"}
    ...
    with c3:
        frame_rate = st.number_input(
            "Video frame rate:", min_value=10, max_value=60, value=30, step=10
        )
        ...
        if alerts == "Value":
            alert = st.number_input(
                "Value below (%):", min_value=50, max_value=100, value=80, step=5
            )
        elif alerts == "Mean":
            alert = st.number_input(
        ...
        interval_start = [
            round(intervals[i] * max_t / len(confidence), 2)
        ...
        interest = st.selectbox(
            "Choose the interval you would like to investigate in:", new_intervals
        )

        values = st.slider(
            "Select a time range (s) for analysis", 0, int(max_t), (0, int(max_t))
        )
        ...
        df_dyn_acc = df_dyn_acc[
            (df_dyn_acc["idx"] >= values[0]) & (df_temp["idx"] <= values[1])
        ]

        st.markdown(
            "<center><h3>Confidence Level</h3></center>", unsafe_allow_html=True
        )
        # st.write("")

        chart_act_conf = (
            alt.Chart(df_dyn_acc)
            .mark_line(color="red")
            .encode(
                x=alt.X(
                    "idx",
                ...
                y=alt.Y("confidence", axis=alt.Axis(title="Confidence")),
            )
            .properties(width=frame_width * 1.5, height=frame_height * 1.3)
        )

        container_act_conf = st.empty()
        container_act_conf.write(
            chart_act_conf.configure_title(fontSize=18).configure_axis(
    ...
    with c2:
        st.markdown(
            "<center><h3>Environmental Video</h3></center>", unsafe_allow_html=True
        )
        ...
        df_video = pd.DataFrame(
            {
                "idx": np.arange(0, number_of_frames / int(fps), 1 / int(fps)),
                "num": np.arange(0, number_of_frames, 1),
            }
        )
    ...
    chart_act = (
        ...
        .encode(
            ...
            y=alt.Y("activity_label", axis=alt.Axis(title="Activity")),
        )
        .properties(width=frame_width * 2.5, height=frame_height * 1.3)
    )

    container_act = st.empty()
    container_act.write(
        chart_act.configure_title(fontSize=18).configure_axis(
            labelFontSize=15, titleFontSize=15
        )
    )
    ...
    dyn_sel = st.multiselect(
        "Select dynamics data:", [dyn_cols[i] for i in range(len(dyn_cols))]
    )
    ...
    chart_dyn = (
        alt.Chart(
            pd.DataFrame(
                {
                    "x": np.arange(int(values[0]), int(values[1]), 1),
                    "y": np.zeros(int(values[1]) - int(values[0])),
                }
            )
        )
        .mark_line(color="white")
        .encode(
            x=alt.X("x", scale=alt.Scale(domain=[int(values[0]), int(values[1])])),
            y=alt.Y("y"),
        )
    )
    ...
    for i in range(len(dyn_cols)):
        if dyn_cols[i] in dyn_sel:
            chart_dyn += (
                alt.Chart(dyn_dict[dyn_cols[i]])
                .mark_line()
                .encode(
                    x=alt.X("idx", axis=alt.Axis(title="Time (s)")),
                    y=alt.Y(dyn_cols[i], axis=alt.Axis(title="Dynamics")),
                    color="key",
                )
                .properties(width=frame_width * 2.5, height=frame_height * 1.3)
            )
    ...
    vit_sel = st.multiselect(
        "Select vitals data:", [vit_cols[i] for i in range(len(vit_cols))]
    )
    ...
    for i in range(len(vit_cols)):
        if vit_cols[i] in vit_sel:
            chart_vit += (
                alt.Chart(vit_dict[vit_cols[i]])
                .mark_line()
                .encode(
                    x=alt.X("idx", axis=alt.Axis(title="Time (s)")),
                    y=alt.Y(vit_cols[i], axis=alt.Axis(title="Vitals")),
                    color="key",
                )
                .properties(width=frame_width * 2.5, height=frame_height * 1.3)
            )
    ...
    dyn_sel_1 = st.multiselect(
        "Select dynamics data to download:",
        [dyn_cols[i] for i in range(len(dyn_cols))],
        [dyn_cols[i] for i in range(len(dyn_cols))],
    )
    ...
    for i in range(len(dyn_cols)):
        if dyn_cols[i] in dyn_sel_1:
            df_dyn_download = pd.concat(
                [df_dyn_download, dyn_dict[dyn_cols[i]]], axis=1
            )

    if st.button("Download dynamics data as CSV"):
        tmp_download_link = download_link(
            df_dyn_download,
            "{}_dynamics.csv".format(patient_name),
            "Click here to download your file",
        )
        st.markdown(tmp_download_link, unsafe_allow_html=True)

    vit_sel_1 = st.multiselect(
        "Select vitals data to download:",
        [vit_cols[i] for i in range(len(vit_cols))],
        [vit_cols[i] for i in range(len(vit_cols))],
    )
    ...
    for i in range(len(vit_cols)):
        if vit_cols[i] in vit_sel_1:
            df_vit_download = pd.concat(
                [df_vit_download, vit_dict[vit_cols[i]]], axis=1
            )

    if st.button("Download vitals data as CSV"):
        tmp_download_link = download_link(
            df_vit_download,
            "{}_vitals.csv".format(patient_name),
            "Click here to download your file",
        )
        st.markdown(tmp_download_link, unsafe_allow_html=True)
    ...
    if st.button("Download classification results as CSV"):
        tmp_download_link = download_link(
            df_dyn_acc_download,
            "{}_activity.csv".format(patient_name),
            "Click here to download your file",
        )
        st.markdown(tmp_download_link, unsafe_allow_html=True)
    ...
    while start and frame_current <= frame_end:
        ...
        container_dyn.write(
            chart_d.configure_title(fontSize=18).configure_axis(
                labelFontSize=15, titleFontSize=15
            )
        )
        ...
        container_vit.write(
            chart_v.configure_title(fontSize=18).configure_axis(
                labelFontSize=15, titleFontSize=15
            )
        )
        ...
        container_act.write(
            chart_a.configure_title(fontSize=18).configure_axis(
                labelFontSize=15, titleFontSize=15
            )
        )
