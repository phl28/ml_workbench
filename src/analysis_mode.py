...
def main():
    ...
    if file_dyn is not None and file_vit is not None and file_vid is not None:
        ...
        with c0:
            st.image("./img/graphic_wide.png", width=600)
            st.write(
                "Platform for clinicians to visualise and analyse dizziness-related data."
            )

        with c1:
            values = st.slider(
                "Select a time range (s) for classification",
                0,
                int(max_t),
                (0, int(max_t)),
            )
        ...
        for i in range(len(dyn_cols)):
            ...
            df_temp = df_temp[
                (df_temp["idx"] >= values[0]) & (df_temp["idx"] <= values[1])
            ]
            dyn_dict["{}".format(dyn_cols[i])] = df_temp
        ...
        with c3:
            frame_rate = st.number_input(
                "Frame rate:", min_value=10, max_value=60, value=30, step=10
            )
            # Dynamics
            ...
            chart_dyn = (
                ...
                .encode(
                    x=alt.X(
                        "x", scale=alt.Scale(domain=[int(values[0]), int(values[1])])
                    ),
                    y=alt.Y("y"),
                )
            )
            ...
            for i in range(len(dyn_cols)):
                if dyn_cols[i] in dyn_sel:
                    chart_dyn += (
                        ...
                        .properties(width=frame_width, height=frame_height)
                    )
        ...
        st.markdown(
            "<center><h3>Neural network (Activity Classifier)</h3></center>",
        ...
        chart_act = (
            ...
            .properties(width=frame_width * 2, height=frame_height * 1.3)
        )
