...
# st.sidebar.header("User setting")
# upload_files = st.sidebar.file_uploader("Upload Dynamics Data", accept_multiple_files= True, type = ['csv'])
# # print(upload_files)
...
# with placeholder.container():
#     task1 = st.selectbox("Select Task", ["-", "Train model", "Update model"], key = 1)
#     flag = True
#     while flag:
#         placeholder = st.empty()

#         if task1 == "Train model":
#             st.write("This is train model")
#             flag = False
#             new_task = st.selectbox("Select Task", ["-", "Train model", "Update model"], key = count)
#         elif task1 == "Update model":
#             st.write("This is update model")
#             flag = False
#             new_task = st.selectbox("Select Task", ["-", "Train model", "Update model"], key = count)
#         if new_task != "-":
#             task1 = new_task
...
#     real_path_2 = st.file_uploader("Choose the model you would like to use", type = ['pkl'])

# if real_path_2 is None and real_path_1 != '-':
#     real_path = real_path_1
#     model = load_dnn_model(real_path)
# elif real_path_2 is not None:
...
#                 conf_value = values
#             else:
#                 conf_value = tuple(interest)

#             chart_act_conf = alt.Chart(df_dyn_acc).mark_line(color = 'red').encode(x = alt.X('idx', axis = alt.Axis(title = 'Time (s)'), scale = alt.Scale(domain = [int(conf_value[0]), int(conf_value[1])])), y = alt.Y('confidence', axis = alt.Axis(title = 'Confidence'))).properties(width = frame_width * 1.5)
